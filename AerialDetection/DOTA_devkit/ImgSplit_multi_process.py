...
class splitbase():
    def __init__(self,
                 ...
                 padding=True,
                 num_process=8,
                 valid_classes=None  # Valid class list
                 ):
        """
        ...
        :param valid_classes: list of valid class names, None means accept all classes
        """
        ...
        # Support CODrone's annotation directory structure
        if os.path.exists(os.path.join(self.basepath, 'annfile')):
            self.labelpath = os.path.join(self.basepath, 'annfile')
        else:
            self.labelpath = os.path.join(self.basepath, 'labelTxt')  # Maintain DOTA compatibility
        ...
        self.valid_classes = valid_classes  # Save valid class list
        print('padding:', padding)
        if self.valid_classes:
            print('Valid classes:', self.valid_classes)
        print('Label path:', self.labelpath)  # Display annotation path in use
    ...
    def savepatches(self, resizeimg, objects, subimgname, left, up, right, down):
        ...
        with codecs.open(outdir, 'w', self.code) as f_out:
            for obj in objects:
                # Class filtering - skip annotations not in valid classes
                if self.valid_classes and obj['name'] not in self.valid_classes:
                    print(f"Skipping unsupported class: {obj['name']}")
                    continue
                ...
                if (half_iou == 1):
                    ...
                    if (len(out_poly) == 5):
                        ...
                        """
                        Skip instances with more than 5 points - not currently handled
                        """
                        continue
                    ...
                    if (half_iou > self.thresh):
                        ...
                        # If the remaining part is too small, label as '2'
    ...
    def splitdata(self, rate):
        ...
        """
        # Count files before splitting
        before_images = len([f for f in os.listdir(self.outimagepath) if f.endswith(self.ext)]) if os.path.exists(self.outimagepath) else 0
        before_labels = len([f for f in os.listdir(self.outlabelpath) if f.endswith('.txt')]) if os.path.exists(self.outlabelpath) else 0
        
        imagelist = GetFileFromThisRootDir(self.imagepath)
        imagenames = [util.custombasename(x) for x in imagelist if (util.custombasename(x) != 'Thumbs')]
        
        # Count original annotation files
        original_labels = len([f for f in os.listdir(self.labelpath) if f.endswith('.txt')]) if os.path.exists(self.labelpath) else 0
        
        print(f"Original images: {len(imagenames)}, Original annotations: {original_labels}")
        ...
        # Count files after splitting
        after_images = len([f for f in os.listdir(self.outimagepath) if f.endswith(self.ext)]) if os.path.exists(self.outimagepath) else 0
        after_labels = len([f for f in os.listdir(self.outlabelpath) if f.endswith('.txt')]) if os.path.exists(self.outlabelpath) else 0
        
        # Calculate newly generated files
        new_images = after_images - before_images
        new_labels = after_labels - before_labels
        
        print(f"Split images: {new_images}, Split annotations: {new_labels}")
