...
class BaseSolver(object):
    ...
    def setup(self, ):
        ...
        # Load tuning weights before EMA
        if self.cfg.tuning:
    ...
    def train(self, ):
        ...
        if self.cfg.resume:
            ...
            print(f"Continue from epoch: {self.last_epoch + 1}")
            if hasattr(self, 'best_map'):
                print(f"Current best mAP: {self.best_map:.4f}")
    ...
    def load_state_dict(self, state):
        ...
        if getattr(self, 'last_epoch', None) is not None and 'last_epoch' in state:
            ...
            print(f'Loading last_epoch: {self.last_epoch}')
        ...
        if 'epoch' in state:
            ...
            if hasattr(self, 'last_epoch'):
                if self.last_epoch != loaded_epoch:
                    print(f'Warning: last_epoch ({self.last_epoch}) != epoch ({loaded_epoch}) in checkpoint')
                    self.last_epoch = loaded_epoch
                    print(f'Corrected last_epoch to: {self.last_epoch}')
