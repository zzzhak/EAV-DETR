...
def get_contrastive_denoising_training_group(targets,
                                             ...
                                             box_noise_scale=1.0,):
    """Build contrastive denoising queries and masks."""
    if num_denoising <= 0:
    ...
    # Determine box dimension (4 for axis-aligned, 5 for oriented)
    box_dim = 5  # default for oriented detection
    for target in targets:
        if len(target['boxes']) > 0:
            box_dim = target['boxes'].shape[-1]
            break
    
    input_query_class = torch.full([bs, max_gt_num], num_classes, dtype=torch.int32, device=device)
    input_query_bbox = torch.zeros([bs, max_gt_num, box_dim], device=device)
    ...
    # Duplicate to positive/negative groups
    ...
    # Positive indices for denoising
    ...
    if box_noise_scale > 0:
        if box_dim == 4:
            ...
            # Oriented boxes: perturb (cx, cy, w, h) and slightly the angle
            bbox_4d = input_query_bbox[..., :4]
            angles = input_query_bbox[..., 4:]
            
            known_bbox = box_cxcywh_to_xyxy(bbox_4d)
            diff = torch.tile(bbox_4d[..., 2:] * 0.5, [1, 1, 2]) * box_noise_scale
            rand_sign = torch.randint_like(bbox_4d, 0, 2) * 2.0 - 1.0
            rand_part = torch.rand_like(bbox_4d)
            rand_part = (rand_part + 1.0) * negative_gt_mask + rand_part * (1 - negative_gt_mask)
            rand_part *= rand_sign
            known_bbox += rand_part * diff
            known_bbox.clip_(min=0.0, max=1.0)
            bbox_4d = box_xyxy_to_cxcywh(known_bbox)
            
            # Small angle noise (radians)
            angle_noise = (torch.rand_like(angles) - 0.5) * 0.2 * box_noise_scale
            angles = angles + angle_noise
            
            input_query_bbox = torch.cat([bbox_4d, angles], dim=-1)
    ...
    # Matching queries cannot see denoising queries
